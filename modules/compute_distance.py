...
Last edited on Wed Nov 20, 2024 by JG
@author: vivekmodi
"""
...
def compute_distance(pdbfilename,index,conf_df,structure):
    conf_df.at[index, 'APE3-aFasp3-dis'] = 999.0
    conf_df.at[index, 'APE10-DFG4-dis'] = 999.0
    conf_df.at[index, 'APE11-DFG4-dis'] = 999.0
    conf_df.at[index, 'APE12-DFG4-dis'] = 999.0
    conf_df.at[index, 'APE10-DFG5-dis'] = 999.0
    conf_df.at[index, 'APE11-DFG5-dis'] = 999.0
    conf_df.at[index, 'APE12-DFG5-dis'] = 999.0
    conf_df.at[index,'Autoinhibit']=''
    ...
    #Distance APE10-DFG4
    if conf_df.at[index, 'APE10_restype'] != "-" and conf_df.at[index,'DFG4_restype'] != "-": 
        conf_df.at[index,'APE10-DFG4-dis']=distance_atoms(pdbfilename,conf_df.at[index,'Model_id'],conf_df.at[index,'Chain_id'],
                                                        conf_df.at[index,'APE10_num'],conf_df.at[index,'DFG4_num'],'CA' if conf_df.at[index,'APE10_restype']=='G' else 'CB','CA',structure)
    #Distance APE11-DFG4
    if conf_df.at[index, 'APE11_restype'] != "-" and conf_df.at[index,'DFG4_restype'] != "-": 
        conf_df.at[index,'APE11-DFG4-dis']=distance_atoms(pdbfilename,conf_df.at[index,'Model_id'],conf_df.at[index,'Chain_id'],
                                                        conf_df.at[index,'APE11_num'],conf_df.at[index,'DFG4_num'],'CA' if conf_df.at[index,'APE11_restype']=='G' else 'CB','CA',structure)
    #Distance APE12-DFG4
    if conf_df.at[index, 'APE12_restype'] != "-" and conf_df.at[index,'DFG4_restype'] != "-": 
        conf_df.at[index,'APE12-DFG4-dis']=distance_atoms(pdbfilename,conf_df.at[index,'Model_id'],conf_df.at[index,'Chain_id'],
                                                        conf_df.at[index,'APE12_num'],conf_df.at[index,'DFG4_num'],'CA' if conf_df.at[index,'APE12_restype']=='G' else 'CB','CA',structure)

    #Distance Ala[APE]-Trp[aF] (not used anywhere currently, but could be used to identify activation-loop swapped structures - JG)
    if conf_df.at[index, 'APE3_restype'] != "-" and conf_df.at[index,'aFasp3_restype'] != "-": 
        conf_df.at[index,'APE3-aFasp3-dis']=distance_atoms(pdbfilename,conf_df.at[index,'Model_id'],conf_df.at[index,'Chain_id'],
                                                        conf_df.at[index,'APE3_num'],conf_df.at[index,'aFasp3_num'],'CA','CA',structure)
    #Distance Glu4-Phe
    ...
    dfgnum=conf_df.at[index,'Asp_num']
    apenum=conf_df.at[index,'APE_num']
    hrdaspnum=conf_df.at[index,'HRDAsp_num']
    autoinhibitdist=999.0
    autoinhibitresidue=0
    autoinhibitohatom='none'
    autoinhibitaspatom='none'
    (actloop_residues,average_bfactor, min_bfactor, max_bfactor)=actloop_bfactors(pdbfilename, conf_df.at[index,'Model_id'], conf_df.at[index,'Chain_id'], dfgnum, apenum, structure)
    conf_df.at[index,'ActLoopBfacAve']=average_bfactor
    ...
    for i in range(int(dfgnum)+5,int(apenum)-7):
        for atom1 in ('OD1','OD2'):
            for atom2 in ('OG','OG1','OH'):
                ...
                dist=distance_atoms(pdbfilename,conf_df.at[index,'Model_id'],conf_df.at[index,'Chain_id'],
                                    hrdaspnum, i,atom1,atom2,structure)
                if dist<autoinhibitdist:
                    autoinhibitdist=dist
                    autoinhibitresidue=i
                    autoinhibitaspatom=atom1
                    autoinhibitohatom=atom2
    if autoinhibitdist<4.0:  conf_df.at[index,'Autoinhibit']=f'{autoinhibitresidue}  {autoinhibitaspatom}  {autoinhibitohatom} {autoinhibitdist:.3}'
    else:
        conf_df.at[index,'Autoinhibit']=''
    return conf_df


def actloop_bfactors(pdbfilename, model_id, chain_id, res1, res2, structure):
    atom_present=0
    res1=int(res1)
    res2=int(res2)
    ...
    for model in structure:
        ...
        for chain in model:
            ...
            for residue in chain:
                ...
                if int(residue.id[1]) >= res1-insertion_num and int(residue.id[1]) <= res2-insertion_num:
                    if residue.has_id('CA'):
                        atom=residue['CA']
                        bfac=atom.get_bfactor()
                        sum=sum + bfac
                        if bfac<minbfac: minbfac=bfac
                        if bfac>maxbfac: maxbfac=bfac
                        count += 1
            ...
            if count>0:
                ...
                avebfac= 999.0
    return count,avebfac, minbfac, maxbfac
