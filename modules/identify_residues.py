...
Last edited on Wed Dec 4, 2024 by JG
@author: vivekmodi
"""
...
def identify_residues(pdbfilename,index,conf_df, s2cdict):
    ...
    # conserved Asp in aF helix
    aFasp= {'AGC':182, 'BUB':196, 'CAMK':181, 'CK1':187, 'CMGC':187, 'EIF2AK41':204, 'HASP':258, 'MAP3K123':175, 'MOS':208,
            'NEK':184, 'OTHER':185, 'PAN3':215, 'PEAK':267, 'PIK3R4':217, 'PINK1':289, 'PKDCC':210, 'POMK':208,
            'PXK':197, 'RGC':184, 'RNASEL':168, 'RPS6KC1':653, 'SCYL':189, 'STE':182, 'STK31':210, 'TBCK':186, 'TKL':186,
            'TP53RK':170, 'TYR':187, 'ULK':181, 'WNK':185, 'CDC7':347, 'ATM':499}
    ...
    aFasp3 = {} #conserved Trp/Tyr/Phe in the alphaF helix (two residues after the conserved Asp)
    ape9={}
    ape10={}
    ape11={}
    ape12={}
        ...
        for hsps in hits:
            ...
            for hmm_res in hsps.aln[0]:
                ...
                if hmm_index==ape[group]:
                    ...
                    conf_df.at[index,'APEp2_num']=s2cnum+2
                    conf_df.at[index,'APEp2_restype']=hsps.aln[1][col_num+1].upper()
                    conf_df.at[index,'APEp3_num']=s2cnum+3
                    conf_df.at[index,'APEp3_restype']=hsps.aln[1][col_num+2].upper()
                    ...
                    for i in range(0,20):
                        ...
                        if rescount == 3 and conf_df.at[index, f'APE{rescount}_num'] == 0:
                            conf_df.at[index, f'APE{rescount}_num']=s2cnum-rescount+1
                            conf_df.at[index,f'APE{rescount}_restype']=hsps.aln[1][col_num-1-i].upper()

                        if rescount > 5 and rescount < 13 and conf_df.at[index, f'APE{rescount}_num'] == 0:
                            conf_df.at[index, f'APE{rescount}_num']=s2cnum-rescount+1
                            conf_df.at[index,f'APE{rescount}_restype']=hsps.aln[1][col_num-1-i].upper()

                if hmm_index==aFasp[group]:
                    conf_df.at[index,'aFasp_num']=s2cnum
                    conf_df.at[index, 'aFasp_restype']=hsps.aln[1][col_num-1].upper()
                    rescount=0
                    for i in range(0,5):
                        if hsps.aln[1][col_num-1+i] != "-":
                            rescount = rescount+1
                        ...
                        if rescount==3 and conf_df.at[index, 'aFasp3_num'] == 0: #Not used anywhere currently
                            conf_df.at[index, 'aFasp3_num']=s2cnum+rescount-1
                            conf_df.at[index,'aFasp3_restype']=hsps.aln[1][col_num-1+i].upper()
                            break
                ...
                if hmm_index==hrdasp[group]:
                    conf_df.at[index,'HRDAsp_num']=s2cnum
                    conf_df.at[index,'HRDAsp_restype']=hsps.aln[1][col_num-1].upper()
                ...
                if hmm_index==asp[group]:
                    ...
                    for i in range(0,20):
                        ...
                        if rescount == 4 and conf_df.at[index,f'DFG{rescount}_num'] == 0: 
                            conf_df.at[index,f'DFG{rescount}_num']=s2cnum+rescount-1
                            conf_df.at[index,f'DFG{rescount}_restype']=hsps.aln[1][col_num-1+i].upper()

                        if rescount == 6 and conf_df.at[index,f'DFG{rescount}_num'] == 0: 
                            conf_df.at[index,f'DFG{rescount}_num']=s2cnum+rescount-1
                            conf_df.at[index,f'DFG{rescount}_restype']=hsps.aln[1][col_num-1+i].upper()
